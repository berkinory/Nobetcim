...
import { Map as MapGL, type MapRef, type ViewStateChangeEvent } from 'react-map-gl/maplibre';
...
function getMapStyleUrl(styleId: string): string {
    ...
    const styleMap = styleId === 'custom' 
        ? '01976890-9b0b-705a-b6ef-fd4f77dd0100' 
        : styleId;
...
function hasLocationChangedSignificantly(
    ...
    return latDiff > MAP_CONFIG.LOCATION.CHANGE_THRESHOLD || lonDiff > MAP_CONFIG.LOCATION.CHANGE_THRESHOLD;
}
...
const MapComponent = forwardRef<MapHandle, MapProps>(({ mapStyle, pharmacies, onLocationFound, initialLocation }, ref) => {
    const [viewState, setViewState] = useState({
        longitude: initialLocation?.longitude ?? 0,
        latitude: initialLocation?.latitude ?? 0,
        zoom: initialLocation ? MAP_CONFIG.ZOOM.INITIAL : MAP_CONFIG.ZOOM.DEFAULT
    });
    ...
    const maxBounds = useMemo(() => [
        [MAP_CONFIG.TURKEY_BOUNDS.west, MAP_CONFIG.TURKEY_BOUNDS.south],
        [MAP_CONFIG.TURKEY_BOUNDS.east, MAP_CONFIG.TURKEY_BOUNDS.north]
    ] as [[number, number], [number, number]], []);

    const processLocationUpdate = useCallback((newLocation: { latitude: number; longitude: number }) => {
        if (!hasLocationChangedSignificantly(userLocation, newLocation)) {
            return;
        }

        console.log('Processing significant location change:', newLocation);
    ...
    const handleLocationUpdate = useCallback((newLocation: { latitude: number; longitude: number }) => {
        const now = Date.now();
        const timeSinceLastUpdate = now - lastLocationUpdateRef.current;

        pendingLocationRef.current = newLocation;

        if (timeSinceLastUpdate >= MAP_CONFIG.LOCATION.UPDATE_THROTTLE_MS) {
            ...
            if (!locationUpdateTimeoutRef.current) {
                const remainingTime = MAP_CONFIG.LOCATION.UPDATE_THROTTLE_MS - timeSinceLastUpdate;
    ...
    const handleMapLoad = useCallback(() => {
        if (initialLocation && mapRef.current && !hasPerformedInitialZoomRef.current) {
    ...
    const zoomIn = useCallback(() => {
        mapRef.current?.zoomIn({ duration: MAP_CONFIG.ANIMATION.ZOOM_CONTROL_DURATION });
    }, []);

    const zoomOut = useCallback(() => {
        mapRef.current?.zoomOut({ duration: MAP_CONFIG.ANIMATION.ZOOM_CONTROL_DURATION });
    }, []);
    ...
    const flyToUserLocation = useCallback(() => {
        if (userLocation && mapRef.current) {
            mapRef.current.flyTo({
                ...
                zoom: Math.max(mapRef.current.getZoom(), MAP_CONFIG.ZOOM.TARGET),
    ...
    const nearbyPharmacies = useMemo(() => {
        ...
        return pharmacies
            ...
            .filter(({ distance }) => distance <= MAP_CONFIG.LOCATION.PHARMACY_RADIUS_KM)
